import { Schema, model, Query, Document, QueryOptions, Types } from "mongoose";
...
const PostSchema = new Schema<PostInterface>({
  ...
  pineCount: { type: Schema.Types.Number, default: 0 },
  createdEmbedding: { type: Schema.Types.Boolean, default: false },
});

function autoPopulateReplies(this: Query<any, Document>, next: () => void) {
  ...
  this.populate([
    ...
    {
      path: "userId",
      select: "_id username displayName reputation avatar userIdHash verified",
      options,
    },
  ]);
  next();
...
function autoPopulatePreviews(this: Query<any, Document>, next: () => void) {
  const options = this.getOptions();
  this.populate([
    {
      path: "preview",
